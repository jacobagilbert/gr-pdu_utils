      ...
      d_wall_clock_time(false)
{
    ...
    this->d_logger->info("starting at time {{{}% {}}}", d_known_time_int_sec, d_known_time_frac_sec);

    this->d_logger->info("rate {:.12f}", d_samp_rate);
...
void tags_to_pdu_impl<T>::handle_ctrl_msg(pmt::pmt_t ctrl_msg)
{
    ...
    if (pmt::dict_has_key(ctrl_msg, PMTCONSTSTR__eob_offset())) {
        ...
        this->d_logger->info("command received - set EOB tag offset to {} symbols", d_eob_offset);
    }
    ...
    if (pmt::dict_has_key(ctrl_msg, PMTCONSTSTR__eob_alignment())) {
        ...
        if (new_eob_alignment > 0) {
            ...
            this->d_logger->info("command received - set EOB tag alignment to {} symbols", d_eob_alignment);
        } else {
                              ...
                              gr_vector_void_star& output_items)
{
    ...
    if (d_triggered) {
        ...
        if (d_tag_type == SOB) {
            ...
            // warning necessary...
            this->d_logger->info("received unexpected EOB at offset {}", d_tag.offset);
